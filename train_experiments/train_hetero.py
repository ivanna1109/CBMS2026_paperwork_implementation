...
# Dodajemo putanju do tvojih modula
...
# Preusmeravanje ispisa u log fajl
...
def train_and_eval_hetero(data, model_type='GAT'):
    # 1. Split podataka (samo na relaciji drug-treats-disease)
    ...
    # Prebacivanje podataka na GPU/CPU unapred
    train_data = train_data.to(DEVICE)
    val_data = val_data.to(DEVICE)
    test_data = test_data.to(DEVICE)

    # 2. Inicijalizacija modela
    # hidden_channels i out_channels moraju biti usklađeni sa HeteroEncoder-om
    ...
    # OVO JE FALILO: Inicijalizacija promenljivih pre petlje
    best_val_auc = 0
    ...
    for epoch in range(1, 101):
        ...
        # Forward pass (koristimo već prebačene x_dict i edge_index_dict)
        z_dict = model(train_data.x_dict, train_data.edge_index_dict)
        
        # Uzimanje labela i indeksa za trening
        ...
        # 3. Evaluacija na svakih 10 epoha
        if epoch % 10 == 0:
            model.eval()
            with torch.no_grad():
                # Validacioni set
                ...
                # Čuvanje najboljeg modela i testiranje
                if val_auc > best_val_auc:
                    best_val_auc = val_auc
                    
                    # Testni set (pokrećemo ga samo kada nađemo bolji model na validaciji)
...
def get_top_cold_predictions(test_data, test_preds, data_original, drug_map, disease_map, top_n=5):
    # 1. Identifikacija hladnih lekova (degree <= 2)
    ...
    # 2. Mapiranje ID-jeva nazad u originalna imena
    ...
    # Maska za hladne lekove koji su STVARNO bili pozitivni primeri u test setu
    ...
    # Sortiranje po verovatnoći (sigurnost modela)
...
def evaluate_cold_start(test_data, test_preds, data_original):
    # 1. Izračunaj stepen lekova u celom originalnom Drug-Disease datasetu
    # Koristimo originalne ivice (data_original je na DEVICE, pa prebacujemo na cpu)
    edge_index_disease = data_original['drug', 'treats', 'disease'].edge_index[0].cpu()
    num_drugs = data_original['drug'].num_nodes
    drug_degree = degree(edge_index_disease, num_nodes=num_drugs)
    
    # 2. Uzmi indekse lekova koji su u test setu (moraju biti na CPU za indexing)
    test_drug_indices = test_data['drug', 'treats', 'disease'].edge_label_index[0].cpu()
    test_labels = test_data['drug', 'treats', 'disease'].edge_label.cpu().numpy()
    
    # 3. Filtriraj samo one koji su "Cold Start" (npr. stepen <= 2)
    # cold_mask je niz True/False vrednosti
    ...
    if cold_mask.sum() > 1: # Potrebno bar 2 uzorka za AUC
        # Filtriramo labele i predikcije koristeći masku
        cold_labels = test_labels[cold_mask]
        cold_preds = test_preds[cold_mask]
        
        # Provera da li imamo obe klase (0 i 1) u hladnom podskupu
...
def run_experiment():
    ...
    # Učitavanje integrisanog grafa (Drug-Gene-Disease)
    data = get_hetero_data(CHG_PATH, DCH_PATH)
    
    # Pokrećemo eksperiment (možeš dodati i SAGE ako implementiraš u hetero_gnn)
    model_types = ['GAT'] 
